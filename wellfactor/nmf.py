...
class NMF_Base(object):
    ...
    default_max_time: float = np.inf

    def __init__(self):
        raise NotImplementedError(
            'NMF_Base is a base class that cannot be instantiated')

    def set_default(self, default_max_iter: int, default_max_time: float) -> None:
        self.default_max_iter = default_max_iter
        self.default_max_time = default_max_time

    def run(self, A: np.ndarray, k: int, init: Optional[Tuple[np.ndarray, np.ndarray]] = None,
            max_iter: Optional[int] = None, max_time: Optional[float] = None, verbose: int = 0) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
...
class NMF_ANLS_BLOCKPIVOT(NMF_Base):
    ...
    def __init__(self, default_max_iter: int = 50, default_max_time: float = np.inf) -> None:
...
def _compare_nmf(m=300, n=300, k=10):
    ...
    names = [NMF_MU, NMF_HALS, NMF_ANLS_BLOCKPIVOT,
             NMF_ANLS_AS_NUMPY]
    iters = [2000, 1000, 100, 100]
    labels = ['mu', 'hals', 'anls_bp', 'anls_as_numpy']
    styles = ['-x', '-o', '-+', '-s']
...
def _test_nmf(m=300, n=300, k=10):
    ...
    alg_names = [NMF_ANLS_BLOCKPIVOT, 
                 NMF_ANLS_AS_NUMPY, NMF_HALS, NMF_MU]
    iters = [50, 50, 500, 1000]

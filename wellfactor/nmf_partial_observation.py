...
from wellfactor.nnls_partial_observation import nnlsm_blockpivot_partial_observation
from wellfactor import matrix_utils as mu

class PartialObservationNMF:
    """ Class implementing Non-negative Matrix Factorization (NMF) for matrices with partial observation. """
    ...
    def run(self, A: Union[np.ndarray, sps.spmatrix], k: int, init: Optional[Tuple[np.ndarray, np.ndarray]] = None, 
            max_iter: Optional[int] = None, max_time: Optional[int] = None, verbose: int = 0, fully_observed_feature_num=None, observed_idx=None) -> Tuple[np.ndarray, np.ndarray, dict]:
        """ Run a NMF algorithm
        ...
        permute_pattern = np.array(
            [i for i in np.arange(A.shape[1]) if i in observed_idx] + \
            [i for i in np.arange(A.shape[1]) if i not in observed_idx]
                                   )
        inverse_pattern = np.argsort(permute_pattern)
        A_permuted = A[:, permute_pattern]
        observed_item_num = len(observed_idx)
        if init is not None:
            W = init[0].copy()
            H = init[1][permute_pattern,:].copy()
        ...
        for i in range(1, info['max_iter'] + 1):
            ...
            (W, H) = self.iter_solver(A_permuted, W, H, k, i, fully_observed_feature_num, observed_item_num)
            elapsed = time.time() - start_iter

            if verbose >= 1:
                rel_error = mu.norm_fro_err_mask(A_permuted, W, H, norm_A, fully_observed_feature_num, observed_item_num) / norm_A
                his['iter'].append(i)
